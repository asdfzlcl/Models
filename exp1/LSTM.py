...
for step in range(EPOCHS):
    ...
    cell = cell.to(DEVICE)
    prediction, h_state,cell = model(Tx, h_state,cell, N1 - 1)  # rnn output
    # 这一步非常重要
    ...
    if (step + 1) % 2 == 0:  # 每训练20个批次可视化一下效果，并打印一下loss
        h_state = h_state.to(DEVICE)
        cell = cell.to(DEVICE)
        prediction, h_state,cell = model(x, h_state,cell, N2 - 1)  # rnn output
        # 这一步非常重要
        h_state = h_state.data  # 重置隐藏层的状态, 切断和前一次迭代的链接
        cell = torch.zeros(1, H_SIZE)
